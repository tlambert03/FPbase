...
function updateGradients(){
  ...
  for (var i=0; i < data.length; i++){
    if (!data[i].disabled){
      if (data[i].gradient){
        $('.nv-groups g.nv-group.nv-series-' + n).find('path').attr('fill','url(#wavecolor_gradient)');
        $($($('.nv-legend g.nv-series')[i]).find('circle')[0]).css({'stroke': 'url(#wavecolor_gradient)', 'fill': 'url(#wavecolor_gradient)'});
      }
      else {
        $('.nv-groups g.nv-group.nv-series-' + n).find('path').removeAttr('fill');
      }
      n++
    }
  }
...
$("body").on('change', '.data-selector', function (event) {
  ...
  // then update the chart
    ...
    addItem(slug).then(function(){
      ...
      if (localData[slug].map(function(e) { return e.type }).indexOf('2p') > -1) {
          row.find('input.2pcheck').closest('td').show();
          // if the 2p button was already checked... keep 2p enabled
          qq = dataItemMatching({slug: slug, type:'2p'})[0];
...
var fluorRow = function(widget) { return $('<tr>', {'class': 'fluor-row'})
                    .append($('<td>')
...
var excRow = function(widget, cls) { return $('<tr>', {'class': cls})
                    .append($('<td>')
                      .append($('<button>', {'class': 'btn btn-danger btn-sm remove-row'})
                        .html('<strong>&times;</strong>')
                      )
...
var filterRow = function(widget, stype) { return $('<tr>', {'class': 'filter-row', 'id': stype + uniqueID(), 'data-ftype': stype})
                    .append($('<td>')
                      .append($('<button>', {'class': 'btn btn-danger btn-sm remove-row'})
                        .html('<strong>&times;</strong>')
                      )
...
var formSelection = function(filter){
  ...
  var selWidget = $('<select>', {
      class: 'form-control form-control-sm custom-select custom-select-sm data-selector ' + classtypes[filter.category]
    });
...
function updateGlobalGradient(){
  ...
  for (var w=cmin; w<cmax; w+=20){
    var stop = document.createElementNS(chartsvgNS, 'stop');
