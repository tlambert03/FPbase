...
$(function(){
  $("#id_parent_organism").siblings('.input-group-append').find('.select-add-button').click(function(){
    $('#organismModal').modal();
  })

  $('#taxonomyModalForm').submit(function(e) {
    form = $(this)
    tax_id = form.find('input[name="taxonomy_id"]').val()
    tax_uri = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=taxonomy&retmode=json&id='
    $.ajax({
      url: tax_uri + tax_id,
      success: function(data){
        if ('scientificname' in data.result[data.result.uids[0]]){
          // successful fetch from NCBI
          sci_name = data.result[data.result.uids[0]]['scientificname'];
          $.ajax({
            type: "POST",
            url: form.attr("data-action-url"),
            data: form.serialize(),
          ...
          $("#hint_id_taxonomy_id").text('Could not find that Taxonomy id at NCBI!')
