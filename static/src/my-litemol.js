...
function initLiteMol(selection, changer) {
  import(/* webpackChunkName: "LiteMol" */ "./js/pdb/LiteMol-plugin.js").then(
    ({ default: LiteMol }) => {
      ...
      try {
        ...
        changer.change(function() {
          ...
          plugin.clear();
          dataCache[id].then(
            data => plugin.loadMolecule({ data, id }),
            reason => { $(selection).html('<span class="text-danger muted">\
              failed to retrieve molecular structure\
            </span>') }
            );
        });
...
function getPDBbinary(id) {
  return new Promise(function(resolve, reject) {
    $.get("https://www.ebi.ac.uk/pdbe/static/entry/" + id.toLowerCase() + "_updated.cif")
      .done(response => {
        resolve(response);
      })
...
function loadSmiles(pdbid) {
  import(/* webpackChunkName: "smilesDrawer" */ "smiles-drawer").then(
    ({ default: smilesDrawer }) => {
      ...
      if (canv.data("chemId") != pdb_info[pdbid].chemId) {
        SmilesDrawer.parse(pdb_info[pdbid].smiles, function(tree) {
          canv.remove();
          canv = $("<canvas>", { id: "smilesCanvas" }).appendTo(
            $("#smilesDrawing .card-img-top")
          );
