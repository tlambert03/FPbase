    ...
    function plot(xvar, yvar, data) {
      ...
      function filtercheck(currentData) {
        for (var f in filters) {
          if (f === "agg") {
            if (filters[f]) {
              if (currentData[f] !== filters[f]) {
                return false
              }
            }
          } else {
            var v = filters[f]
            if (currentData[f] < v[0] || currentData[f] > v[1]) {
              return false
            }
          }
        }
      ...
      var datagroup = svg.selectAll("g.FP").data(currentData, function(d) {
        return d.name
      })
      var entergroup = datagroup
        .enter()
        .append("g")
        .attr("class", "FP")
      ...
      datagroup.each(function(d, i) {
        ...
        current
          ...
          .transition()
          .attr("x", function(d) {
            return xScale(d[xvar]) - symbolsize
          })
          .attr("y", function(d) {
            return yScale(d[yvar]) - symbolsize
          })
          .duration(circle_move_duration) //change this number to speed up or slow down the animation
        current
          .selectAll("text.FP")
          .transition()
          .attr("x", function(d) {
            return xScale(d[xvar]) - symbolsize / 2
          })
          .attr("y", function(d) {
            return yScale(d[yvar]) + symbolsize / 2
          })
          .duration(circle_move_duration) //change this number to speed up or slow down the animation
      })
      ...
      .on("mouseover", function(d) {
        ...
        if (d3.select(this).attr("cx")) {
          ...
          d3.select(this)
            ...
            .duration(100)
            .attr("x", function(d) {
              return xScale(d[currentX]) - symbolsize * bigscale
            })
            .attr("y", function(d) {
              return yScale(d[currentY]) - symbolsize * bigscale
            })
