...
;(function(){
    ...
    var svg = d3.select('.microscope-wrapper #spectra svg');
        ...
        if (!(concatslug in localData)) {
            ...
            slugs.forEach(function(s, ind, array) {
                asyncFunction(s, function(){
                    itemsProcessed += 1;
                    if(itemsProcessed === array.length) {
                        var names = slugs.map(function(o) { return localData[o][0].key }).reduce(function(a, b){ return a + "+" + b })
    ...
    function removeSubtypes(subtype) {
        var ix = $.map(data, function(obj, index) {
            if (obj.type == subtype) {
                return index;
            }
        });
        ...
        if (slug != null && slug != 0) {
            addItem(slug).then(function() {
                ...
                if (localData[slug].map(function(e) { return e.type; }).indexOf(CONST.stype.twop) > -1) {
                    ...
                    var item2p = dataItemMatching({ slug: slug, type: CONST.stype.twop })[0]; // assume only 1
