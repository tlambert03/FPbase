...
function fpbaseLink(accession) {
    return (
        <a href={`/protein/${accession}`} target="_blank">
            {accession}
        </a>
    );
}
...
function BlastReportDescription({ report, onClick }) {
    const query = {
        id: report.search.query_id,
        title: report.search.query_title,
        len: report.search.query_len,
    };
                        ...
                        <TableRow key={row.key}>
                            <TableCell align="left">
                                <a
                                    ...
                                    href={row.accession}
                                    onClick={onClick}
                                    title={`show alignment of query with ${
                                        row.title
                                    }`}
                                >
                                    {row.title}
                            ...
                            <TableCell align="right">
                                ...
                                {Math.round(
                                    (1000 * row.identity) / row.align_len
                                ) / 10}
                                %)
                            </TableCell>
                            <TableCell align="right">{row.gaps}</TableCell>
                            <TableCell align="right">
                                {fpbaseLink(row.accession)}
                            </TableCell>
                        </TableRow>
                    ))}
                </TableBody>
