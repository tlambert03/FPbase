/**
 * Spectrum Chart Module
 *
 * Creates Highcharts-based spectrum preview with click-to-set-peak support.
 */
...
export function createSpectrumChart(container, data, options = {}) {
  ...
  const chart = Highcharts.chart(containerId, {
    chart: {
      ...
      spacing: [10, 10, 25, 10], // top, right, bottom, left
      events: {
        click: (e) => {
          if (onClick && e.xAxis && e.xAxis[0]) {
            const wavelength = e.xAxis[0].value
            onClick(wavelength)
          }
        },
      },
  ...
  const controller = {
    ...
    updateData(newData, seriesName = name) {
      ...
      // Update x-axis range
      const wavelengths = newData.map(([x]) => x)
      const newMin = Math.min(...wavelengths)
      const newMax = Math.max(...wavelengths)
      chart.xAxis[0].setExtremes(newMin - 10, newMax + 10)
    },

    /**
     * Set peak marker line.
     */
    setPeakMarker(wavelength) {
      // Remove existing peak marker
      ...
      if (wavelength !== null) {
        chart.xAxis[0].addPlotLine({
          id: "peak-marker",
          value: wavelength,
          color: "#36a83b", // FPbase primary green
          width: 2,
          dashStyle: "Dash",
          label: {
