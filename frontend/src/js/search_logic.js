window.initSearch = function (filterfields, operatorLookup, labelLookup) {
  ...
  let available_fields = new Set(Object.keys(fields))
  let i = 0
  let prevName
  let prevOperator
  ...
  function updateInputField(row, keepValue) {
    ...
    let filter_name = row.find(".filter-select").val()
    let operation = row.find(".operator-select").val()
    if (operation == "exact") {
      name = filter_name
    } else {
    ...
    let inputcol = row.find(".input-col")
    if (keepValue) {
  ...
  function addRow(target, filter, operator) {
    let newrow = $(queryRow(i)).appendTo(target)

    if (filter) {
      newrow.find(".filter-select").val(filter)
    }

    let filterSelector = newrow.find(".filter-select")
    let filterName = filterSelector.val()

    let operatorSelect = newrow.find(".operator-select")
  ...
  function updateOperators(filterName, sender) {
    ...
    let selector = $("." + filterName + "_operator").not(sender)
