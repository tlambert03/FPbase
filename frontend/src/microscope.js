...
window.initMicroscope = () => {
  ...
  ;(() => {
    ...
    const options = {
      ...
      precision: localStorage.getItem(`${KeyPrefix}_precision`) || (isSafari ? 2 : 1),
      interpolate: localStorage.getItem(`${KeyPrefix}_interpolate`) === "true",
      // modified later at page load
      calcEff: true,
      exEffBroadband: localStorage.getItem(`${KeyPrefix}_exEffBroadband`) === "true",
    }
    const userOptions = {
      calcEff: {
        type: "checkbox",
    ...
    function mergeSpectra(concatslug) {
      ...
      if (!(concatslug in localData)) {
        ...
        const asyncFunction = (s, onfinished) => {
          if (s.startsWith("laser")) {
            if (Object.hasOwn(localData, s)) {
        ...
        slugs.forEach((s, _ind, array) => {
          asyncFunction(s, () => {
            itemsProcessed += 1
            if (itemsProcessed === array.length) {
              const names = slugs.map((o) => localData[o][0].key).reduce((a, b) => `${a}+${b}`)
    ...
    function pushData(item) {
      if (options.precision > 1) {
        const newitem = { ...item }
        newitem.values = newitem.values.filter((d) => d.x % options.precision === 0)
        data.push(newitem)
      } else {
        data.push(item)
      }
    ...
    function addMerged(comboslug) {
      return mergeSpectra(comboslug).then((d) => {
        pushData(d[0])
      })
    }
    ...
    function excitationNormalization() {
      if (options.exNormWave) {
        for (let n = 0; n < data.length; n++) {
          if (data[n].type === "em") {
            const exvals = dataItemMatching({ slug: data[n].slug, type: "ex" })[0].values
            let targetScalar = dataItemMatching({ x: options.exNormWave }, exvals)[0].y
            ...
            if (currentScalar !== targetScalar) {
              ...
              data[n].values = data[n].values.map((item) => ({ x: item.x, y: item.y * S }))
    ...
    function resizeYSlider() {
      try {
        const rect = $("#spectrasvg").find(".nv-focus").first().find(".nv-background>rect")
    ...
    $(".2pcheck").change(function (_e) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (type === CONST.stype.twop && localData[slug]) {
          const pos = localData[slug].map((e) => e.type).indexOf("2p")
    ...
    function setup_from_url(urlParams) {
      ...
      if ("c" in urlParams) {
        ...
        $.each(urlParams.f.split(","), (_index, val) => {
          if (val.endsWith("!")) {
            val = val.slice(0, -1)
    ...
    $(() => {
              ...
              $("<input>", {
                ...
                .change(function () {
                  if (value.type === "checkbox") {
                    ...
                    const val = Math.min(Math.max(this.value, value.min), value.max)
                    options[key] = val
      ...
      nv.addGraph(
        () => {
          ...
          chart.dispatch.on("stateChange", (_e) => {
            chart.update()
          })
      ...
      $(".filter-selector, .invswitch").change(() => {
        $("#config-select").val("")
      })
      ...
      $("#config-select").change(function () {
        ...
        $.each(selected.data("filters"), (_i, d) => {
    ...
    $("body").on("click", ".remove-row", function (_e) {
      const row = $(this).closest(".row")
    ...
    function customLaser(wave) {
      return padDataLimits({
        ...
        minwave: wave,
        maxwave: wave,
        values: [{ x: options.precision * Math.round(wave / options.precision), y: 1 }],
      })
    }
    ...
    const defs =
      ...
      chartsvg.insertBefore(document.createElementNS(chartsvgNS, "defs"), svg.firstChild)
    ...
    function efficiency(filtervalues, fluor) {
      ...
      let efficiency
      if (fluor.type === "em") {
        efficiency = filter_dye_area / trapz(fluorspectrum)
      } else if (fluor.type === "ex") {
        if (options.exEffBroadband) {
          efficiency = filter_dye_area / trapz(fluorspectrum)
        } else {
          efficiency = filter_dye_area / trapz(filtervalues)
        }
      }
      ...
      for (_i = 0, _len = elements.length; _i < _len; _i++) {
        ...
        _results.push(
          ((el) => {
            ...
            let _results1
            resizeText = () => {
              const elNewFontSize = `${parseInt($(el).css("font-size").slice(0, -2), 10) - 0.5}px`
    ...
    function selectedSlugs() {
      ...
      let list
      if ($("#merge-light-exfilter").prop("checked") && $("#light-select :selected").val() !== "") {
        list = selected.not("#light-select :selected, .ex-filter:checked").map(function () {
          return $(this).val()
        })
        let _concat = "merged/"
    ...
    function updateEfficiency() {
      ...
      if (L.data("type") === "laser") {
        ...
        let _cameff = 1
        try {
          emeff = a.emvalues.filter((d) => d.x === Lwave || d.x === Lwave + 1)[0].y
        } catch (_e) {
          emeff = null
        }
        try {
          if ($("#scale-camera").prop("checked")) {
            _cameff = dataItemMatching({ category: "c" })[0].values.filter(
              (d) => d.x === Lwave || d.x === Lwave + 1
            )[0].y
          }
        } catch (_e) {}
        if (emeff) {
          const blocking = Math.round(-Math.log10(emeff / _cameff) * 10) / 10
      ...
      if (a.emvalues?.length) {
        emeff = efficiency(a.emvalues, { slug: fluorslug, type: "em" })
      }
      if (a.exvalues?.length) {
        exeff = efficiency(a.exvalues, { slug: fluorslug, type: "ex" })
      }
      let info = ""
      if (exeff) {
        // if (options.normMergedEx)
        ...
        pushData(exeff)
        if (exeff.efficiency) info += `Ex: ${String(Math.round(exeff.efficiency * 1000) / 10)}%`
      }
      if (emeff) {
        pushData(emeff)
        if (emeff.efficiency) {
      ...
      if ((emeff !== undefined) & (exeff !== undefined)) {
        const b = exeff.efficiency * exeff.scalar * emeff.efficiency * emeff.scalar
      ...
      setTimeout(() => {
        ...
        $(".filter-label").each(function (_i) {
          this.innerHTML = this.innerHTML.replace(/^\s*\w+/, (x) => {
            ...
            if (f === "chroma" || f === "semrock" || f === "zeiss" || f === "omega") {
...
// Note: Initialization is now handled by the data-fpbase-init pattern in index.js
// This ensures proper coordination with the main bundle and CDN dependencies
