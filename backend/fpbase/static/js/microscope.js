...
;(() => {
  ...
  function scale_data_down(filter) {
    for (let n = 0; n < data.length; n++) {
      ...
      let skip = false
      for (const key in filter) {
        if (typeof data[n][key] === 'undefined' || data[n][key] != filter[key] || !data[n].scaled) {
          skip = true
          break
        }
      }
  ...
  function setup_from_url(urlParams) {
    ...
    if ('c' in urlParams) {
      ...
      $.each(urlParams.f.split(','), (index, val) => {
        if (val.endsWith('!')) {
          val = val.slice(0, -1)
  ...
  $(() => {
    ...
    nv.addGraph(
      () => {
        ...
        chart.interactiveLayer.tooltip.valueFormatter((d, i) => {
          if (d) {
    ...
    $('#config-select').change(function () {
      ...
      $.each(selected.data('filters'), (i, d) => {
  ...
  function updateEfficiency() {
    ...
    if (L.data('type') === 'laser') {
      ...
      try {
        var emeff = a.emvalues.filter((d) => d.x === Lwave || d.x === Lwave + 1)[0].y
      } catch (e) {
        var emeff = null
      }
      var cameff = 1
