...
def etagged_response(response: HttpResponse, request: HttpRequest, *etag_models: type[Model]) -> HttpResponse:
    ...
    if response.status_code == 200 and request.method == "GET":
        ...
        # Django already adds Date header
    return response


def parse_if_none_match_header(header_value: str | None) -> list[str]:
...
def check_etag_match(
    ...
    *models: type[Model],
    ...
    """Check if request ETag matches current version and return 304 response if so.
    ...
    client_etags = parse_if_none_match_header(if_none_match)
