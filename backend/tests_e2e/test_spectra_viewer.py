...
def test_subtype_visibility_toggles(spectra_viewer: Page) -> None:
    ...
    series_paths = spectra_viewer.locator(".highcharts-series-group path.highcharts-area")
    expect(series_paths).to_have_count(2)  # EGFP has EX and EM spectra

    # Find the subtype toggle buttons within the EGFP selector group
    egfp_selector = tab_wrapper.locator('div[class*="SpectrumSelector"]').filter(has_text="EGFP")
    ex_button = egfp_selector.get_by_role("button", name="EX", exact=True)
    em_button = egfp_selector.get_by_role("button", name="EM", exact=True)
    ...
    expect(series_paths).to_have_count(1)  # Only EM should be visible

    # Click EM button to hide EM spectrum
    em_button.click()
    spectra_viewer.wait_for_timeout(200)
    expect(series_paths).to_have_count(0)  # No spectra visible

    # Click EX button again to show EX spectrum
    ex_button.click()
    spectra_viewer.wait_for_timeout(200)
    expect(series_paths).to_have_count(1)  # EX is visible again

    # Click EM button again to show EM spectrum
    em_button.click()
    spectra_viewer.wait_for_timeout(200)
    expect(series_paths).to_have_count(2)  # Both EX and EM visible again
