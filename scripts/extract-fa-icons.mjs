 ...
 *
 * USAGE:
 *   1. Install dependencies first (only needed once):
 *      pnpm add -D @fortawesome/free-solid-svg-icons @fortawesome/free-regular-svg-icons @fortawesome/free-brands-svg-icons
 *
 *   2. Run the script:
 *      node scripts/extract-fa-icons.mjs
 *
 * IMPORTANT:
 *   - This script will overwrite backend/fpbase/icons.py and frontend/src/icons/fa-icons.ts
 *   - If it fails, it will NOT modify files (safe by default)
 *   - Git can recover files if something goes wrong: git restore backend/fpbase/icons.py frontend/src/icons/fa-icons.ts
 */
...
async function checkDependencies() {
  ...
  for (const pkg of required) {
    try {
      ...
      console.error('\nPlease install Font Awesome dependencies:');
      console.error('  pnpm add -D @fortawesome/free-solid-svg-icons @fortawesome/free-regular-svg-icons @fortawesome/free-brands-svg-icons\n');
      process.exit(1);
    }
  }
...
async function main() {
  // CRITICAL: Check dependencies first to prevent data loss
  await checkDependencies();

  console.log('\nExtracting Font Awesome icons...');
  const icons = await extractIcons();

  // CRITICAL: Validate extraction was successful
  if (icons.length === 0) {
    console.error('\n❌ ERROR: No icons were extracted!');
    console.error('This would overwrite existing icon data with empty files.');
    console.error('Aborting to prevent data loss.\n');
    process.exit(1);
  }

  const expectedCount = Object.keys(iconMap).length;
  if (icons.length < expectedCount * 0.8) {
    console.error(`\n⚠️  WARNING: Only extracted ${icons.length}/${expectedCount} icons`);
    console.error('This seems wrong. Aborting to prevent data loss.\n');
    process.exit(1);
  }
...
main().catch(err => {
  ...
  console.error('\nFiles were NOT modified (safer to fail than corrupt data).\n');
  process.exit(1);
});
